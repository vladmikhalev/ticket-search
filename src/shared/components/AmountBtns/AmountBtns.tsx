...
export function AmountBtns({id}: IPropsAmountBtns) {
  ...
  useEffect(() => {
    ...
    productAmount <= 0 ? setIsDisableBtnDecrease('true') : setIsDisableBtnDecrease('false');
    productAmount >= 30 ? setIsDisableBtnIncrease('true') : setIsDisableBtnIncrease('false');
  }, [productAmount]);

  return (
    <div className={styles.amountBtns}>
      <button className={styles.btnDecrease} data-disabled={isDisableBtnDecrease} onClick={() => dispatch(decrement({id: id}))}>
      ...
      <button className={styles.btnIncrease} data-disabled={isDisableBtnIncrease} onClick={() => dispatch(increment({id: id}))}>
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          <rect width="20" height="20" rx="4" fill="#FF5500" />
