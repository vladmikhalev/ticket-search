...
export function AmountBtns() {
  const [isDisableBtnDecrease, setIsDisableBtnDecrease] = useState('true');
  const [isDisableBtnIncrease, setIsDisableBtnIncrease] = useState('false');
  const [amountTikets, setAmountTikets] = useState(0);

  function handleDecreaseTomatos() {
    if (amountTikets > 0) {
      setAmountTikets(prevAmount => prevAmount - 1)
    }
  }
  ...
  useEffect(() => {
    ...
    amountTikets === 0 ? setIsDisableBtnDecrease('true') : setIsDisableBtnDecrease('false');
    amountTikets === 30 ? setIsDisableBtnIncrease('true') : setIsDisableBtnIncrease('false');
  }, [amountTikets]);

  return (

    <div className={styles.amountBtns}>
      <button className={styles.btnDecrease} data-disabled={isDisableBtnDecrease} onClick={handleDecreaseTomatos}>
      ...
      <span className={styles.amount}>{amountTikets}</span>
      <button className={styles.btnIncrease} data-disabled={isDisableBtnIncrease} onClick={handleIncreaseTomatos}>
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          <rect width="20" height="20" rx="4" fill="#FF5500" />
